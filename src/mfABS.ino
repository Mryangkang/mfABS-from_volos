  ...
  for(int i=0;i<26;i++)
   {
      ...
      //画抗锯齿的断线，将两条线中间分成11个格子 牛皮
      for(int i=0;i<13;i++)
      sprite.drawWedgeLine(x2[i*25],y2[i*25],px2[i*25],py2[i*25],1,1,0xCE59);

      //隔一个绘制一个蓝色或者紫色的加重色点，指示整数用
      if(i%2==0) {
        sprite.drawWedgeLine(x[i*12],y[i*12],px[i*12],py[i*12],3,1,color1);
    
        sprite.setTextColor(color2,backColor);
        sprite.drawString(String(i*10),lx[i*12],ly[i*12]);  //绘制速度码盘数值
      }else
      {
        sprite.drawWedgeLine(x[i*12],y[i*12],px[i*12],py[i*12],1,1,0xCE59);  //整数之间使用白色短线
      }
  }
